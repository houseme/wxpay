 ...
 * Time: 2017/3/29 17:32
 * FileName: WxPayApi.class.php
 * Chinese:
 */
...
class WxPayApi
{
    ...
    public static function report($inputObj, $timeOut = 1)
    {
        ...
        $startTimeStamp = self::getMillisecond();//请求开始时间
        return self::postXmlCurl($xml, $url, false, $timeOut);
    }
    ...
    {
        //如果不需要上报数据
        if (WxPayConfig::getReportLevenl() === 0) {
            return;
        }
        //如果仅失败上报
        if (WxPayConfig::getReportLevenl() === 1 && array_key_exists('return_code', $data) && $data['return_code'] === 'SUCCESS' && array_key_exists('result_code', $data) && $data['result_code'] === 'SUCCESS') {
            return;
        }
    ...
    private static function postXmlCurl($xml, $url, $useCert = false, $second = 30)
    {
        ...
        if ($useCert == true) {
            ...
            curl_setopt($ch, CURLOPT_SSLCERT, WxPayConfig::getSslCertPath());//SSLCERT_PATH);
            curl_setopt($ch, CURLOPT_SSLKEYTYPE, 'PEM');
            curl_setopt($ch, CURLOPT_SSLKEY, WxPayConfig::getSslKeyPath());//SSLKEY_PATH);
        }
        //post提交方式
